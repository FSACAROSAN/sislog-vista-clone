...
const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  return <Link to={path} className={`sidebar-item ${active ? 'active' : ''}`}>
      {icon}
      <span>{label}</span>
    </Link>;
};
...
const SidebarSubItem: React.FC<SidebarItemProps> = ({
  ...
  return <Link to={path} className={`flex items-center gap-2 px-2 py-2 text-sm text-gray-700 rounded-md hover:bg-gray-100 ${active ? 'bg-gray-100 font-medium' : ''}`}>
      {icon}
      <span>{label}</span>
    </Link>;
};
          ...
          <CollapsibleTrigger className="w-full">
            <div className={`sidebar-item ${isConfigActive ? 'active' : ''} justify-between`}>
              <div className="flex items-center">
                <Settings size={18} />
                <span className="px-[10px]">Configuración</span>
              </div>
          ...
          <CollapsibleContent>
            <div className="pl-12 pr-4 mt-1">
              <div className="flex flex-col space-y-1">
                <SidebarSubItem icon={<Building2 size={16} />} label="Empresa" active={location.pathname === '/empresa'} path="/empresa" />
                <SidebarSubItem icon={<Map size={16} />} label="Países" active={location.pathname === '/paises'} path="/paises" />
                <SidebarSubItem icon={<MapPin size={16} />} label="Ciudades" active={location.pathname === '/ciudades'} path="/ciudades" />
                <SidebarSubItem icon={<Warehouse size={16} />} label="Centro Logístico" active={location.pathname === '/centro-logistico'} path="/centro-logistico" />
                <SidebarSubItem icon={<img src="/lovable-uploads/93a7ddd3-b268-4cb6-93ca-eb23d2e21690.png" className="w-4 h-4" alt="Bodega" />} label="Bodegas" active={location.pathname === '/bodegas'} path="/bodegas" />
                <SidebarSubItem icon={<PackageOpen size={16} />} label="Stands" active={location.pathname === '/stands'} path="/stands" />
              </div>
            </div>
          </CollapsibleContent>
        </Collapsible>
